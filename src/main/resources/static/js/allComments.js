...
$(function(){
    ...
    var allComment = {
        appId : 'fLHTlMN2j3',
        baseUrl:'https://api.comment.huanqiu.com/api',
        baseOpt : {
            source_id : getURL('source_id'),
            a_title : article.title,
            category : decodeURIComponent(window.location.host),
            user_name : '',
            user_uid : '',
            user_avatar: '',
        ...
        getAjax:function(setting){
            ...
            $.ajax({
                url:allComment.baseUrl + '/' + opts.url,
                type:'get',
                data:opts.data,
                dataType:'json',
                xhrFields: {
                ...
                success:function(result){
                    opts.successFn && opts.successFn(result);
                    // console.log(result);
                },
                error:function(e){
                    console.log(e);
        ...
        render:function(){
             // 标题
            article.addltype == "gallery" ? article.addltype ="gallery":article.addltype ="article";
            // console.log(article.addltype);
            var tit_url = '//' + window.location.host + '/' + article.addltype + '/' + article.aid;
            //$(".header h1 a").text(allComment.baseOpt.a_title).attr("title",allComment.baseOpt.a_title).attr("href",tit_url);
            // 点赞
            $("body").on("click",".tie-support",function(){
                var _this = $(this),
                    _id = $(this).parents("li").attr("data-id");
                // console.log($(this).parents("li").attr("data-id"));
               allComment.action.actionAjax(_this,_id);
            });
            // 他的跟帖
            $("body").on("click","#huanqiu-tieList .user-name",function(){
                var uid = $(this).attr("data-uid"),
                    avatar = $(this).parents("li").eq(0).find(".user-avatar").find("img").attr("src"),
                    name = escapeHtml($(this).text());
                allComment.tie.myTie(uid,avatar,name);
                $("#huanqiuUserIndexInner").show();
            });
            $("#huanqiu-tieInner").on("click",".user-avatar",function(){
                var uid = $(this).find("img").attr("data-uid"),
                    avatar = $(this).find("img").attr("src"),
                    name = escapeHtml($(this).siblings(".user-content").find(".user-name").eq(0).text());
                allComment.tie.myTie(uid,avatar,name);
                $("#huanqiuUserIndexInner").show();
            });
        },
        ...
        userInfo:function(){
            var requestData = {
                url:'userInfo',
                data:{
                    'app_id':allComment.appId,
                    "type":["comment_statistics"]
                },
                successFn:function(res){
                    if(res.code == 2000){
                        allComment.ready();
                        allComment.baseOpt.loginFlag = true;
                        allComment.baseOpt.user_name = res.data.username;
                        allComment.baseOpt.user_uid = res.data.uid;
                        allComment.baseOpt.user_avatar = res.data.avatar;
                        allComment.baseOpt.user_comment_count = res.data.comment_statistics.comment_count;
                        allComment.baseOpt.user_reply_count = res.data.comment_statistics.reply_count;
                        allComment.logined();
                        allComment.myIfon();
                    }else{
                        allComment.ready();
                        $("body").on("click",".my-avatar,.user-login",function(){
                            allComment.login(window.location.href);
                        });
                        $("body").on("click",".tie-report",function(){
                            var _url = 'http:' + $(this).attr("data-link");
                            allComment.login(_url,"_blank");
                        });
                        $("body .social-login").on("click","a",function(){
                            var _type = $(this).attr("type");
                            allComment.socialLogin(window.location.href,_type);
                        });
                    }
                }
            }
            this.getAjax(requestData);
        },
        // 登录
        login:function(url,type){
            var requestData = {
                url:'auth/login',
                data:{
                    'app_id':allComment.appId,
        ...
        logout:function(){
            $.ajax({
                type: "post",
                url: "https://i.huanqiu.com/uc/auth/logout",
                data: "app_id=" + allComment.appId,
                dataType: "json",
                ...
                success: function () {
                    ...
                    allComment.postAjax(requestData);
                }
            });
        },
        // 登录后相关操作
        logined:function(){
            var loginedHtml = '';
            $(".my-avatar").attr("src",allComment.baseOpt.user_avatar);
            $(".login").hide();
            loginedHtml += '<div class="user-con hover-style">'
            loginedHtml +=     '<div class="user-name">'
            loginedHtml +=         '<a href="javascript:void(0);">'+ allComment.baseOpt.user_name +'</a>'
            loginedHtml +=         '<div class="sign-icon"></div>'
            loginedHtml +=     '</div>'
            loginedHtml +=     '<ul>'
            loginedHtml +=         '<li><a data-uid="'+ allComment.baseOpt.user_uid +'" href="javascript:void(0);" class="logined-user-tie">我的跟贴</a></li>'
            loginedHtml +=         '<li><a class="switch-accounts" href="javascript:void(0);">切换帐号</a></li>'
            loginedHtml +=     '</ul>'
            loginedHtml += '</div>'
            $(".logined").html(loginedHtml);
            $(".logined .user-con").hover(function(){
                $(this).removeClass("hover-style");
            },function(){
                $(this).addClass("hover-style");
            });
            $("body").on("click",".logined-user-tie,.my-info-inner .user-name",function(){
                $("#huanqiuUserIndexInner").show();
                var _uid = $(this).attr("data-uid");
                allComment.tie.myTie(_uid,allComment.baseOpt.user_avatar,allComment.baseOpt.user_name);
            });
            $("body").on("click",".switch-accounts",function(){
                allComment.changeLogin(window.location.href);
            });
        },
        // 我的信息
        myIfon:function(){
            var html = '';
            html+= '<div class="box-inner my-info-inner">'
            html+=     '<div class="inner-tit">'
            html+=         '<div class="tit">我的信息</div>'
            html+=         '<div class="tit-shadow"></div>'
            html+=         '<a class="quit">退出</a>'
            html+=     '</div>'
            html+=     '<div class="con">'
            html+=         '<div class="avatar">'
            html+=             '<img src="'+ allComment.baseOpt.user_avatar +'">'
            html+=         '</div>'
            html+=         '<div class="info">'
            html+=             '<dl>'
            html+=                 '<dt>'+ allComment.baseOpt.user_name +'<a href="https://i.huanqiu.com" target="_blank">修改昵称</a></dt>'
            html+=                 '<dd>'
            html+=                     '<a class="user-name" data-uid="'+ allComment.baseOpt.user_uid +'" href="javascript:void(0);">跟帖&nbsp;'+ allComment.baseOpt.user_comment_count +'</a><span class="sx">|</span><a class="user-name" data-uid="'+ allComment.baseOpt.user_uid +'" href="javascript:void(0);">回复&nbsp;'+ allComment.baseOpt.user_reply_count +'</a>'
            html+=                 '</dd>'
            html+=             '</dl>'
            html+=         '</div>'
            html+=     '</div>'
            html+= '</div>';
            $(".other-inner").prepend(html);
            $("body").on("click",".quit",function(){
                allComment.logout();
            });
        },
        // 最热评论列表
        getHotTieList:function(){
            var requestData = {
                url:'commentHot',
                data:{
                    'app_id':allComment.appId,
                    'source_id':allComment.baseOpt.source_id,
                },
                successFn:function(res){
                    ...
                    if(data.length>0){
                        ...
                        html += '<strong>最热跟贴</strong>'
                        html += '</div>'
                        html += '<ul class="tie-list">'
                        for(var i=0; i<data.length; i++){
                            html += '<li class="tie-box m-need-avatar" data-id="'+ data[i].id +'" data-reply="'+ data[i].reply_count +'">'
                            html +='<div class="user-avatar">'
                            html +=    '<img src="'+ data[i].user_info.avatar +'" class="user-avatar-img" data-uid="'+ data[i].user_info.uid +'">'
                            html +='</div>'
                            html +='<div class="user-content">'
                            html +=    '<div class="tie-header">'
                            html +=        '<div class="post-time">'+ getTime(data[i].created_at) +'</div>'
                            html +=        '<div class="user-info">'
                            html +=            '<a class="user-name" href="javascript:void(0);" data-uid="'+ data[i].user_info.uid  +'">'+ data[i].user_info.username +'</a><span class="ip-location">'+ data[i].loc +'网友</span>'
                            html +=        '</div>'
                            html +=    '</div>'
                            html +=    '<div class="tie-content">'+ data[i].content +'</div>'
                            html +=    '<div class="tie-footer">'
                            html +=        '<span class="operating">'
                            html +=            '<a href="javascript:void(0);" class="tie-support"><var class="red">'+ data[i].like_count +'</var></a>'
                            if(data[i].reply_count == 0){
                            html +=            '<a href="javascript:void(0);" class="tie-reply">回复&nbsp;</a>'
                            }else{
                            html +=            '<a href="javascript:void(0);" class="tie-reply">回复&nbsp;('+ data[i].reply_count +')</a>'
                            }
                            if(allComment.baseOpt.loginFlag){
                                html +=                         '<a href=//'+ window.location.host +'/commentreport?comment_id='+ data[i].id +' target="_blank" class="tie-report" title="举报"><var>举报</var></a>'
                            }else{
                                html +=                         '<a class="tie-report" data-link=//'+ window.location.host +'/commentreport?comment_id='+ data[i].id +' title="举报"><var>举报</var></a>'
                            }
                            html +=        '</span>'
                            html +=    '</div>'
                            html +='</div>'
        ...
        // 最新评论列表
        getNewTieList:function(page,last_id){
            var requestData = {
                url:'commentStatistics',
                data:{
                    'app_id':allComment.appId,
                    'source_id':allComment.baseOpt.source_id,
                    'limit':10
                ...
                successFn:function(res){
                    if(res.code == 2000){
                        $(".participate").html('<var>'+ res.data.statistics.participation_sum +'</var>&nbsp;人参与，<var>'+ res.data.statistics.comment_sum +'</var>&nbsp;条跟帖');
                        if(res.data.statistics.comment_sum > 0){
                            $(".new-tie-inner").show();
                            var data = res.data.comment_data.comments;
                            var html = '';
                            for(var i=0; i<data.length; i++){
                                html += '<li class="tie-box m-need-avatar" data-id="'+ data[i].id +'" data-reply="'+ data[i].reply_count +'">'
                                html +='<div class="user-avatar">'
                                ...
                                if(data[i].top_comment.length>0){
                                    ...
                                    html +=     '<ul>'
                                    for(var t=0; t<data[i].top_comment.length; t++){
                                        html +=         '<li class="tie-box child m-need-avatar" data-reply="'+ data[i].top_comment[t].is_top +'" data-id="'+ data[i].top_comment[t].id +'">'
                                        html +=             '<div class="user-avatar">'
                                        html +=                 '<img data-uid="'+ data[i].top_comment[t].user_info.uid +'" src="'+ data[i].top_comment[t].user_info.avatar +'" class="user-avatar-img">'
                                        html +=             '</div>'
                                        html +=             '<div class="user-content">'
                                        html +=                 '<div class="tie-header">'
                                        html +=                     '<div class="post-time">'+ getTime(data[i].top_comment[t].created_at) +'</div>'
                                        html +=                     '<div class="user-info">'
                                        html +=                         '<a class="user-name" href="javascript:void(0);" data-uid="'+ data[i].top_comment[t].user_info.uid  +'">'+ data[i].top_comment[t].user_info.username +'</a><span class="ip-location">'+ data[i].top_comment[t].loc +'网友</span>'
                                        html +=                     '</div>'
                                        html +=                 '</div>'
                                        html +=                 '<div class="tie-content">回复 <span>'+ data[i].user_info.username+ '</span>: ' + data[i].top_comment[t].content  +'</div>'
                                        html +=                 '<div class="tie-footer">'
                                        html +=                     '<span class="operating">'
                                        html +=                         '<a href="javascript:void(0);" class="tie-support"><var class="red">'+ data[i].top_comment[t].like_count +'</var></a>'
                                        html +=                         '<a href="javascript:void(0);" class="tie-reply">回复</a>'
                                        if(allComment.baseOpt.loginFlag){
                                            html +=                         '<a href=//'+ window.location.host +'/commentreport?comment_id='+ data[i].top_comment[t].id +' target="_blank" class="tie-report" title="举报"><var>举报</var></a>'
                                        }else{
                                            html +=                         '<a class="tie-report" data-link=//'+ window.location.host +'/commentreport?comment_id='+ data[i].top_comment[t].id +' title="举报"><var>举报</var></a>'
                                        }
                                        html +=                     '</span>'
                                        html +=                 '</div>'
                                        html +=             '</div>'
                                    ...
                                    html +=     '</ul>'
                                    if(data[i].reply_count > 2){
                                        html += '<div class="get-more-reply">加载更多回复</div>'
                                    }
                                    html += '</div>'
                                }
                                html +='</div>'
                            ...
                            $(".new-tie-inner .tie-list").append(html);
                            if(data.length >= 10){
                                var more = '<div class="get-more-list">共有<var>'+ res.data.statistics.participation_sum +'</var>人参与，查看更多精彩跟帖 &gt;&gt;</div>'
                                if($(".get-more-list").length == 0){
                                    $(".new-tie-inner .tie-list").after(more);
                                }
                            }else{
                                $("body .get-more-list").remove();
                                if($(".go-to-top").length == 0){
                                    $(".new-tie-inner .tie-list").after('<div class="go-to-top"><a href="javascript:scroll(0,0)">此贴已全部展示，点击返回顶部</a></div>');
                                }
                            };
                            $("body .get-more-list").off("click").on("click",function(){
                                var _last = $(".new-tie-inner .tie-list").children("li").last().attr("data-id");
                                allComment.getNewTieList(2,_last);
                            });
                            $("#huanqiu-tieList").off("click").on("click",".get-more-reply",function(){
                                var _last = $(this).parents(".children-list").find("li:last").attr("data-id"),
                                    _dom = $(this).parents(".children-list").find("ul"),
                                    comment_id = $(this).parents("li.tie-box").attr("data-id");
                                if(!$(this).parents(".children-list").hasClass("one")){
                                    allComment.reply.getReplysListMore(1,_dom,comment_id);
                                }else{
                                    allComment.reply.getReplysListMore(2,_dom,comment_id,_last);
                                }
                            });
                        }
                    }
            ...
            if(page != 1){
                requestData.data.last_id = last_id;
            }
            this.getAjax(requestData);
        },
        // 热帖排行
                    ...
                    if(res.code == 2000){
                        ...
                        if(res.data.length){
                            for(var i=0; i<res.data.length; i++){
                                ...
                                html +=         '<a href="'+ res.data[i].url +'" target="_blank" title="'+ res.data[i].title +'">'+ res.data[i].title  +'</a>'
                                html +=         '<span>（'+ res.data[i].participation_sum +'人参与）</span>'
                                html +=     '</div>'
                                html += '</li>';
                            }
                            $(".hot-ranking-inner ul").html(html);
                        }
                    }
                }
            }
        ...
        addComment:function(){
            ...
            $('.tie-textarea-box').focus(function (e) { 
                e.preventDefault();
                $(this).addClass("write-box");
            });
            $('.tie-textarea-box').blur(function (e) { 
                e.preventDefault();
                if($('.tie-textarea-box').val().length == 0){
                    $(this).removeClass("write-box");
                }
            });
            $(".release-tie").on("click",function(){
                var nowDate = parseInt(new Date().getTime() / 1000);
                if(!$(".release-tie").hasClass("disabled") && allComment.baseOpt.loginFlag){
                    var con = $('.tie-textarea-box').val().replace(/<[^>]+>/g,"");
                    // console.log(con);
                    if(nowDate - allComment.baseOpt.postTieTime >= 5){
                       allComment.addCommentAjax(con);
                        $(".top .operating-inner .error").remove();
                    }else{
                        // console.log("间隔超过5秒");
                        $(".top .operating-inner .error").remove();
                        $(".top .operating-inner").prepend('<div class="error">还有' + (5 - (nowDate - allComment.baseOpt.postTieTime)) + '秒后才能继续评论！</div>');
                    }
                }else{
                    if(!allComment.baseOpt.loginFlag&&userSession==null){
                    	document.location.href="login"
                    }
                }
            });
            $('.top .tie-textarea-box').keyup(
                function(e){
                    var remain = $(this).val().length;
                    // console.log(remain);
                    if(remain > 0 && remain <= 500){
                        $(".top .release-tie").removeClass("disabled"); 
                        $(".top .text-num-tips").remove();   
                    }
                    else{
                        $(".top .operating-inner").prepend('<div class="text-num-tips">跟帖须（2-500）字内</div>');
                        $(".top .release-tie").addClass("disabled");        
                    }
                    if(remain == 0 ){
                        $(".top .text-num-tips").remove();
                    }
        ...
        // 发帖请求
        addCommentAjax:function(content){
            var requestData = {
                url:'comment',
                data:{
                    'app_id':allComment.appId,
                ...
                successFn:function(res){
                   allComment.baseOpt.postTieTime = parseInt(new Date().getTime() / 1000);
                    if(res.code == 2000){
                        var html = '';
                        html += '<li class="tie-box m-need-avatar" data-id="'+ res.data.comment_id +'" data-reply="0">'
                        html +='<div class="user-avatar">'
                        html +=    '<img src="'+ allComment.baseOpt.user_avatar +'" class="user-avatar-img" data-uid="'+ allComment.baseOpt.user_uid +'">'
                        html +='</div>'
                        html +='<div class="user-content">'
                        html +=    '<div class="tie-header">'
                        html +=        '<div class="post-time">刚刚发布</div>'
                        html +=        '<div class="user-info">'
                        html +=            '<a class="user-name" href="javascript:void(0);" data-uid="'+ allComment.baseOpt.user_uid  +'">'+ allComment.baseOpt.user_name +'</a><span class="ip-location"></span>'
                        html +=        '</div>'
                        html +=    '</div>'
                        html +=    '<div class="tie-content">'+ content +'</div>'
                        html +=    '<div class="tie-footer">'
                        html +=        '<span class="operating">'
                        html +=        '此跟帖已发布成功'
                        html +=        '</span>'
                        ...
                        html +='</li>';
                        $(".new-tie-inner").show();
                        $(".new-tie-inner .tie-list").prepend(html);
                        $('html,body').animate({scrollTop:$(".new-tie-inner").offset().top}, 100);
                        $('.top .tie-textarea-box').val("");
                        $(".release-tie").addClass("disabled");
                    }else if(res.code == 3004){
                        $(".top .operating-inner").prepend('<div class="error">评论内容不可重复</div>');
            ...
            this.postAjax(requestData);
        },
        // 点赞
        action:{
            actionAjax:function(dom,comment_id){
                var requestData = {
                    url:'action',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                       allComment.action.addSupport(dom);
                    }
                }
                allComment.postAjax(requestData);
            },
            addSupport:function(e){
                if(!e.hasClass("support-disabled")){
                    e.addClass("support-disabled");
                    var val = parseInt(e.children(":first").html());
                    e.append('<div class="add-support">+1</div>');
                    var $addSupport = e.children().eq(1);
                    var opacity = 1;
                    $addSupport.css("opacity ",opacity);
                    $addSupport.animate({top:"-28px",opacity:0});
                    e.children(":first").html(val + 1);
                }
            }
        },
        // 回复
        reply:{
            // 获取回复
            getReplysList:function(dom,comment_id){
                var requestData = {
                    url:'replys',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                        if(res.code == 2000){
                            var data = res.data.replys;
                            var html = '';
                            html += '<div class="children-list one">'
                            html +=     '<ul>'
                            for(var i=0; i<data.length; i++){
                                html +=         '<li class="tie-box child m-need-avatar" data-reply="" data-id="'+ data[i].id +'">'
                                html +=             '<div class="user-avatar">'
                                html +=                 '<img data-uid="'+ data[i].user_info.uid +'" src="'+ data[i].user_info.avatar +'" class="user-avatar-img">'
                                html +=             '</div>'
                                html +=             '<div class="user-content">'
                                html +=                 '<div class="tie-header">'
                                html +=                     '<div class="post-time">'+ getTime(data[i].created_at) +'</div>'
                                ...
                                html +=                     '</div>'
                                html +=                 '</div>'
                                html +=                 '<div class="tie-content">回复 <span>'+ data[i].resp_user_info.username + '</span>: ' + data[i].content  +'</div>'
                                html +=                 '<div class="tie-footer">'
                                html +=                     '<span class="operating">'
                                html +=                         '<a href="javascript:void(0);" class="tie-support"><var class="red">'+ data[i].like_count +'</var></a>'
                                html +=                         '<a href="javascript:void(0);" class="tie-reply">回复</a>'
                ...
                allComment.getAjax(requestData);
            },
            // 获取回复更多
            getReplysListMore:function(type,dom,comment_id,last_id){
                var requestData = {
                    url:'replys',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                        if(res.code == 2000){
                            ...
                            if(data.length){
                                ...
                                if(type == 1){
                                    $(dom).parents(".children-list").addClass("one");
                                    $(dom).find("li").remove();
                                }
                                dom.append(html);
                                var le = $(dom).parents(".children-list").find("li").length
                                if(res.data.count == le){
                                    $(dom).siblings(".get-more-reply").remove();
                                }
                            }
                        }
                    }
            ...
            // 提交回复
            addReplyCommentAjax:function(content,respid,name){
                var requestData = {
                    url:'reply',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                        if(res.code == 2000){
                            ...
                            html +=        '</div>'
                            html +=    '</div>'
                            html +=    '<div class="tie-content">@'+ name + '&nbsp' + content +'</div>'
                            html +=    '<div class="tie-footer">'
                            html +=        '<span class="operating">'
                            html +=        '此跟帖已发布成功'
                            html +=        '</span>'
                            ...
                            html +='</li>';
                            $(".new-tie-inner .tie-list").prepend(html);
                            $(".add-panel").parents("li").removeClass("reply-z-index");
                            $(".add-panel").remove();
                            $('html,body').animate({scrollTop:$(".new-tie-inner").offset().top}, 100);
                        }else if(res.code == 3004){
                            $(".reply-tips").parents(".operating-inner").prepend('<div class="error">评论内容不能重复</div>');
                        }else if(res.code == 4001){
                            window.location.reload();
                        }else if(res.code == 3005){
                            alert(res.msg);
            ...
            replyComment:function(){
                $("body").on("click",".tie-reply",function(){
                    var _name = $(this).parents(".tie-footer").siblings(".tie-header").find(".user-name").text();
                    // $(this).parents("li.tie-box").siblings().find(".add-panel").remove();
                    $(this).parents(".tie-list").find(".add-panel").not(this).remove();
                    if(!$(this).parents("li.tie-box").eq(0).hasClass("reply-z-index")){
                        $(this).parents("li.tie-box").eq(0).addClass("reply-z-index").siblings("li").removeClass("reply-z-index");
                        $(this).parents(".tie-footer").after(allComment.reply.replyHtml(_name));
                    }else{
                        $(this).parents("li.tie-box").eq(0).removeClass("reply-z-index");
                        $(".add-panel").remove();
                    };
                    $("#huanqiu-tieList .add-panel .tie-textarea-box").keyup(
                        function(e){
                            var remain = $(this).val().length;
                            if(remain > 0 && remain <= 500){
                                $("#huanqiu-tieList .release-tie").removeClass("disabled"); 
                                $("#huanqiu-tieList .text-num-tips").remove();   
                            }
                            else{
                                $("#huanqiu-tieList .operating-inner").prepend('<div class="text-num-tips">评论内容最多输入500字</div>');
                                $("#huanqiu-tieList .release-tie").addClass("disabled");        
                            }
                            if(remain == 0 ){
                                $("#huanqiu-tieList .text-num-tips").remove();
                            }  
                    ...
                    $('#huanqiu-tieList').find('.user-con').hover(function(){
                            $(this).removeClass("hover-style");
                        },function(){
                            $(this).addClass("hover-style");
                    });
                    $(".logined .user-name").attr("disabled",true).css("pointer-events","none"); 
                });
                $("body .hot-tie-inner").on("click",".tie-reply",function(){
                    // console.log("hot");
                    var comment_id = $(this).parents("li.tie-box").eq(0).attr("data-id"),
                        dom = $(this).parents(".user-content").eq(0);
                    if(!$(this).parents("li.tie-box").eq(0).hasClass("reply-z-index")){
                        dom.find(".children-list").remove();
                        allComment.reply.getReplysList(dom,comment_id);
                    }else{
                        dom.find(".children-list").remove();
                    }
                    
                });
                $("body").on("click",".reply-tips",function(){
                    if(!$(this).hasClass("disabled") && allComment.baseOpt.loginFlag){
                        var nowDate = parseInt(new Date().getTime() / 1000);
                        var con = $('.tie-list .tie-textarea-box').val().replace(/<[^>]+>/g,""),
                            name = $(".add-panel").siblings(".tie-header").find(".user-name").text(),
                            id = $(".add-panel").parents("li").attr("data-id");
                        // console.log(id);
                        if(nowDate - allComment.baseOpt.postTieTime >= 5){
                            allComment.reply.addReplyCommentAjax(con,id,name);
                            $(this).parents(".operating-inner").find(".error").remove();
                        }else{
                            // console.log("间隔超过5秒");
                            $(this).parents(".operating-inner").find(".error").remove();
                            $(this).parents(".operating-inner").prepend('<div class="error">还有' + (5 - (nowDate - postTieTime)) + '秒后才能继续评论！</div>');
                        }
                    }else{
                        if(!allComment.baseOpt.loginFlag){
                            allComment.login(window.location.href);
            ...
            // 回复框
            replyHtml:function(name){
                var html = '';
                html +=  '<div class="posting-panel-inner add-panel" id="huanqiu-tiePost">'
                html +=    '<div class="post-textarea-inner">'
                html +=        '<div class="post-textarea">'
                html +=            '<div class="post-textarea-inner">'
                html +=                '<textarea placeholder="回复：'+ name +'" autocomplete="off" name="postTextarea" class="tie-textarea-box"></textarea>'
                html +=            '</div>'
                html +=            '<div class="operating-inner clearfix">'
                if(allComment.baseOpt.loginFlag){
                    html +=                '<div class="login hidden">' 
                    html +=                    '<a class="user-login" href="javascript:void(0);">登录</a><span class="sx"> | </span><a href="https://i.huanqiu.com/register" target="_blank">注册</a></div>'
                    html +=                    '<div class="logined">'
                    html +=                         '<div class="user-con hover-style" >'
                    html +=                             '<div class="user-name">'
                    html +=                                 '<a href="javascript:void(0);">'+ allComment.baseOpt.user_name +'</a><div class="sign-icon"></div>'
                    html +=                             '</div>'
                    html +=                             '<ul>'
                    html +=                                 '<li>'
                    html +=                                     '<a data-uid="'+ allComment.baseOpt.user_uid +'" href="javascript:void(0);" class="logined-user-tie">我的跟贴</a>'
                    html +=                                 '</li>'
                    html +=                                 '<li><a class="switch-accounts" href="javascript:void(0);">切换帐号</a></li>'
                    html +=                             '</ul>'
                    html +=                         '</div>'
                    html +=                    '</div>'
        ...
        tie:{
            // 我的跟帖操作
            myTie:function(uid,avatar,name){
                var html = '';
                html += '<div class="user-info-inner">'
                if(uid == allComment.baseOpt.user_uid){
                    html += '<img class="avatar" src="'+ allComment.baseOpt.user_avatar +'" data-uid="'+ allComment.baseOpt.user_uid +'">'
                }else{
                    html += '<img class="avatar" src="'+ avatar +'" data-uid="'+ uid +'">'
                }
                html += '<div class="km-inner">'
                html +=     '<div class="user-tie">'
                if(uid == allComment.baseOpt.user_uid){
                    html +=         '<var>'+ allComment.baseOpt.user_comment_count +'</var>&nbsp;条跟帖，<var>'+ allComment.baseOpt.user_reply_count +'</var>&nbsp;条回复' 
                }else{
                    html +=         '<var>0</var>&nbsp;条跟帖，<var>0</var>&nbsp;条回复'
                }
                html +=     '</div>'
                html +=     '<div class="user-tit">'
                if(uid == allComment.baseOpt.user_uid){
                    html +=         '<strong>'+ allComment.baseOpt.user_name +'</strong> 的跟帖'
                }else{
                    html +=         '<strong>'+ name +'</strong> 的跟帖'
                }
                html +=     '</div>'
                html += '</div>'
                html += '</div>'
                ...
                if(uid == allComment.baseOpt.user_uid){
                    if(!allComment.baseOpt.loginFlag){
                        html +=         '<li class="active"><a class="user-index-popup-tie me" href="javascript:void(0);">我的跟帖</a></li>'
                    }else{
                        html +=         '<li class="active"><a class="user-index-popup-tie me" href="javascript:void(0);">我的跟帖</a></li>'
                        html +=         '<li><a class="user-index-popup-reply" href="javascript:void(0);">回复我的</a></li>'
                    }
                }else{
                    html +=         '<li class="active"><a class="user-index-popup-tie ta" href="javascript:void(0);">他的跟帖</a></li>'
                }
                html +=     '</ul>'
                html += '</div>'
                html += '<div class="tie-list">'
                html +=     '<ul></ul>'
                html += '</div>'
                html += '</div>'
                html += '<a class="close" href="javascript:void(0);" title="关闭"></a>'
                $("#huanqiu-userIndex").html(html);
                $(".close").on("click",function(){
                    $("#huanqiuUserIndexInner").hide();
                    $("#huanqiu-userIndex").html("");
                });
                $(".tie-list-tab .nav ul").on("click","li",function(){
                    allComment.baseOpt.scroll = 0;
                    $(".tie-list-tab .tie-list ul").html("");
                    $(this).addClass("active").siblings().removeClass("active");
                    if($(this).find("a").hasClass("user-index-popup-reply")){
                        // myTieAjax('replyTie',1);
                        allComment.tie.myReplyAjax(1);
                    }else if($(this).find("a").hasClass("me")){
                        allComment.tie.myTieAjax(1);
                    }else if($(this).find("a").hasClass("ta")){
                        allComment.tie.taTieAjax(1,uid,name);
                    }
                });
                if(uid == allComment.baseOpt.user_uid){
                    allComment.tie.myTieAjax(1);
            ...
            // 我的跟帖列表
            myTieAjax:function(page,last_id){
                var requestData = {
                    url:'comment/history',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                        var data = res.data.commemts;
                        if(res.code == 2000){
                            var html = '';
                            for(var i=0; i<data.length; i++){
                                html+= '<li class="tie-box" data-id="'+ data[i].id +'">'
                                html+=    '<div class="user-content">'
                                html+=        '<div class="tie-header">'
                                html+=            '<div class="post-time">'+ getTime(data[i].created_at) +'</div>'
                                html+=            '<div class="user-info">'
                                html+=                '<a class="user-name-index" href="javascript:void(0);" data-uid="'+ allComment.baseOpt.user_uid +'">'+ allComment.baseOpt.user_name +'</a><span class="ip-location">'+ data[i].loc +'网友</span>'
                                html+=            '</div>'
                                html+=        '</div>'
                                html+=         '<div class="tie-content">'
                                if(data[i].original.original_comment){
                                    html+=             '<div class="original-tie-inner">'
                                    html+=                 '<div class="tie-header">'
                                    html+=                     '<div class="user-info">'
                                    html+=                         '<div class="post-time">'
                                    html+=                             getTime(data[i].original.original_comment.original_created_at)
                                    html+=                         '</div><a class="user-name-index" href="javascript:void(0);" data-uid="'+ data[i].original.user_info.uid +'">'+ data[i].original.user_info.username +'</a><span class="ip-location">'+ data[i].original.original_comment.original_loc +'网友</span>'
                                    html+=                     '</div>'
                                    html+=                 '</div>'
                                    html+=                 '<div class="tie-content">'+ data[i].original.original_comment.original_comment +'</div>'
                                    html+=             '</div>'
                                    html+=             '<a href="javascript:void(0);" class="reply-user-name"><em>@</em>'+ data[i].original.user_info.username +'</a>&nbsp'
                                }
                                html+=             data[i].content
                                html+=         '</div>'
                                html+=        '<div class="tie-footer">'
                                html+=            '<span class="origin-title">来自：<a href="'+ data[i].source_info.url +'" target="_blank" title="'+ data[i].source_info.title +'">'+ data[i].source_info.title +'---'+ data[i].status_description +'</a></span>'
                                html+=        '</div>'
                                html+=    '</div>'
                                html+= '</li>'
                            }
                            $(".tie-list-tab .tie-list ul").append(html);
                            if(data.length == 10){
                               allComment.baseOpt.scroll = 1;
                            }else{
                                allComment.baseOpt.scroll = 0;
                            }
                            allComment.tie.scrollTie('myTie');
                        }
                    }
                }
                if(page != 1){
            ...
            // 他的跟帖列表
            taTieAjax:function(page,uid,name,last_id){
                var requestData = {
                    url:'comment/user',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                        ...
                        if(res.code == 2000){
                            ...
                            for(var i=0; i<data.length; i++){
                                ...
                                html+=            '<div class="post-time">'+ getTime(data[i].created_at) +'</div>'
                                html+=            '<div class="user-info">'
                                html+=                '<a class="user-name-index" href="javascript:void(0);" data-uid="'+ uid +'">'+ name +'</a><span class="ip-location">'+ data[i].loc +'网友</span>'
                                html+=            '</div>'
                                html+=        '</div>'
                                html+=         '<div class="tie-content">'
                                if(data[i].original.original_comment){
                                ...
                                html+=         '</div>'
                                html+=        '<div class="tie-footer">'
                                html+=            '<span class="origin-title">来自：<a href="'+ data[i].source_info.url +'" target="_blank" title="'+ data[i].source_info.title +'">'+ data[i].source_info.title +'</a></span>'
                                html+=        '</div>'
                                html+=    '</div>'
                                html+= '</li>'
                            }
                            $("#huanqiu-userIndex .user-tie").html('<var>'+ res.data.comment_count +'</var>&nbsp;条跟帖，<var>'+ res.data.reply_count +'</var>&nbsp;条回复');
                            $(".tie-list-tab .tie-list ul").append(html);
                            if(data.length == 10){
                                allComment.baseOpt.scroll = 1;
                            }else{
                            ...
                            allComment.tie.scrollTie('taTie',uid); 
                        }
                    }
                }
                if(page != 1){
            ...
            // 我的回复列表
            myReplyAjax:function(page,last_id){
                var requestData = {
                    url:'reply/history',
                    data:{
                        'app_id':allComment.appId,
                    ...
                    successFn:function(res){
                        ...
                        if(res.code == 2000){
                            ...
                            for(var i=0; i<data.length; i++){
                                ...
                                html+=             '<div class="post-time">'+ getTime(data[i].created_at) +'</div>'
                                html+=             '<div class="user-info">'
                                html+=                 '<a class="user-name-index" href="javascript:void(0);" data-uid="'+ data[i].res_user_info.uid +'">'+ data[i].res_user_info.username +'</a><span class="ip-location">'+ data[i].loc +'网友</span>'
                                html+=             '</div>'
                                html+=         '</div>'
                                html+=         '<div class="tie-content">'
                                html+=             '<div class="original-tie-inner">'
                                ...
                                html+=                     '<div class="user-info">'
                                html+=                         '<div class="post-time">'
                                html+=                             getTime(data[i].original_comment.original_created_at)
                                html+=                         '</div><a class="user-name-index" href="javascript:void(0);" data-uid="'+ allComment.baseOpt.user_uid +'">'+ allComment.baseOpt.user_name +'</a><span class="ip-location">'+ data[i].original_comment.original_loc +'网友</span>'
                                html+=                     '</div>'
                                html+=                 '</div>'
                                html+=                 '<div class="tie-content">'+ data[i].original_comment.original_comment +'</div>'
                                html+=             '</div><a href="javascript:void(0);" class="reply-user-name"><em>@</em>'+ allComment.baseOpt.user_name +'</a>&nbsp'+ data[i].content +''
                                html+=         '</div>'
                                html+=         '<div class="tie-footer">'
                                html+=             '<span class="origin-title">来自：<a href="'+ data[i].source_info.url +'" target="_blank" title="'+ data[i].source_info.title +'">'+ data[i].source_info.title +'</a></span>'
                                html+=         '</div>'
                            ...
                            allComment.tie.scrollTie("replyTie"); 
                        }
                    }
                }
                if(page != 1){
            ...
            // 滚动
            scrollTie:function(type,uid){
                var _name = $("#huanqiu-userIndex .user-info-inner .km-inner .user-tit strong").text();
                $("#huanqiu-userIndex .tie-list-tab .tie-list").off('resize scroll').on('resize scroll',function(){
                    var windowHeight = $("#huanqiu-userIndex .tie-list-tab .tie-list").height();//当前窗口的高度             
                    var scrollTop = Math.ceil($("#huanqiu-userIndex .tie-list-tab .tie-list").scrollTop());//当前滚动条从上往下滚动的距离            
                    var docHeight = $("body #huanqiu-userIndex .tie-list-tab .tie-list ul").height(); //当前文档的高度 
                    // console.log(windowHeight+'/'+scrollTop+"/"+docHeight);
                    if (scrollTop + windowHeight >= docHeight) {
                        var le_id = $("#huanqiu-userIndex .tie-list-tab .tie-list ul li").last().attr("data-id");
                        if(allComment.baseOpt.scroll == 1){
                            // console.log(le_id);
                            if(type == "myTie"){
                                allComment.tie.myTieAjax(2,le_id);
                            }else if(type == "replyTie"){
                                allComment.tie.myReplyAjax(2,le_id);
                            }else if(type == "taTie"){
                                allComment.tie.taTieAjax(2,uid,_name,le_id);
                            }
                            allComment.baseOpt.scroll = 0;
                        } 
    ...
    function getTime(t){
        ...
        var thisTime = parseInt(time_str);
        var ot = new Date(parseInt(thisTime)),
        ct = (new Date()).getTime()-parseInt(ot.getTime()),cttext,
        ctminute = Math.floor(ct/60000);
        if(ctminute<1){
            cttext = '刚刚发表';
        }else if(ctminute < 60){
            cttext = ctminute + '分钟前';
        }else if(ctminute < 1440){
            cttext = Math.round(ctminute / 60) + '小时前';
        }else if(ctminute < 2880){
            cttext = '1天前';
        }else if(ctminute < 8640){
            cttext = parseInt(ctminute / 1440) + '天前';
        }else if(ctminute < 17280){
            cttext = '1周前';
        }else if(ctminute < 34560){
            cttext = '2周前';
        }else if(ctminute < 69120){
            cttext = '3周前';
        }else{
            cttext = ot.getFullYear() + '年' + (ot.getMonth() + 1) + '月' + (ot.getDate()) + '日';
        }
        return cttext;  
    }
